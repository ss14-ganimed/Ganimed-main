...
#pragma warning restore RA0003
{
    ...
    private void OnMessageAdded(ChatMessage msg)
    {
        ...
        AddLine(ProcessedWrappedMessage(msg.WrappedMessage, msg.SenderEntity), color);
    }

    private string ProcessedWrappedMessage(string wrappedMessage, EntityUid senderEntity)
    {
        if (!_entityManager.HasComponent<MetaDataComponent>(senderEntity))
        {
        ...
        if (_entityManager.HasComponent<GhostComponent>(_playerManager.LocalPlayer?.ControlledEntity)
            ...
            && wrappedMessage.Contains(name))
        {
            wrappedMessage = wrappedMessage.Replace(name,
                                                $"{name} [cmdlink=\"F\" command=\"orbit {senderEntity}\"][/cmdlink]");
        }
